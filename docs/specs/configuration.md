...
当连接的 MCP 客户端声明了 `roots` 能力（如 Claude Code）时，Sight
MCP 会在初始化完成后自动向客户端请求工作区根目录，并将其纳入允许路径集合，无需弹出授权对话框。

- 请求在后台以 3 秒超时进行，失败（协议不支持、超时、客户端未声明能力）都静默降级，不影响服务可用性。
- 仅接受 `file://` 协议的根目录，其他 scheme 被忽略。
- 返回的路径经过 `realpath` 规范化，必须是真实存在的目录。
- 文件系统根 `/` 或整个用户主目录会被拒绝并记录警告，不纳入允许集合。
- 嵌套的根被归并为最小等价集合，行为和 `SIGHT_ALLOWED_ROOTS` 一致。
- 客户端可以通过 `notifications/roots/list_changed` 通知服务刷新根目录列表。

**注意：** `roots/list`
请求在 MCP 协议 2026-07-28 版本（SEP-2577）已被标记废弃，SDK 在新协议 era 下不会发送该请求。此功能是可选增强，不是免弹窗的唯一手段，服务在客户端不支持时完全正常工作。

### 会话内授权缓存

当读取的路径同时位于配置根目录和客户端工作区根目录之外时，Sight
MCP 会弹出 macOS 授权对话框。用户点击 Allow 后，该路径的**父目录**会被记录到进程内存缓存中，同一会话中该目录下的其他文件不再弹窗。

- 缓存仅存活于进程内存，进程重启后清空——不落盘、不持久化。
- 缓存以目录为粒度，批准 `~/Downloads/a.png` 只缓存 `~/Downloads`，不会向上提升到 `~`。
- 文件系统根 `/` 或整个用户主目录不会被缓存，无论路径深度如何。
- 读取被取消时（信号中止），不会残留缓存记录。
- 点击 Deny 不会缓存，下次读取同一路径继续弹窗。

这一层是协议无关的，也是客户端不支持根目录自动发现时的兜底免弹窗方案。
