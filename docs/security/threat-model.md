...
- Scope: local stdio server, local image input, `sharp` preprocessing, one OpenAI-compatible vision
  provider, built-in Provider profiles, and optional macOS Keychain credentials
...
7. With a built-in profile, the selected API key is requested from the exact Keychain service and
...
credential backend, or a change to profile/credential precedence.
