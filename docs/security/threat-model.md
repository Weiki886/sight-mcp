...
- Date: 2026-08-28
- Runtime risk: High
- Scope: local stdio server, local image input, `sharp` preprocessing, one OpenAI-compatible vision provider
- Related: [Proposal 0001](../proposals/0001-sight-mcp-v0.1.0.md), [tool contract](../specs/vision-tool-contract.md), [configuration](../specs/configuration.md)
...
3. Keep credentials, local paths, image bytes, prompts, and raw upstream responses out of logs and public errors.
4. Bound CPU, memory, file descriptors, queue growth, network duration, retries, response size, and provider cost amplification.
...
- Local filesystem: contains authorized and unauthorized data and may contain links or special files.
...
- Sight MCP cannot protect data from an attacker who already controls the same account, process environment, MCP host, or provider.
...
| IMG-03 | Misleading extension or polyglot | Parser confusion, bypass | Ignore extension for trust; require successful bounded decode and allowlisted decoded format | Mismatch, malformed, and polyglot fixtures |
...
4. Source and normalized buffers become unreachable after request completion/cancellation and are not cached.
5. The server writes no prompt, answer, image, or usage history to disk.
6. Provider-side retention is outside Sight MCP's control and must be assessed by the user when choosing a remote endpoint.

JavaScript cannot guarantee immediate zeroization of all copies. The design minimizes lifetime and duplication but does not claim secure erasure from managed memory.
...
- Provider work requires a high-risk label, credential/redaction tests, destination-policy tests, and no live secrets.
...
- A known critical decoder or runtime vulnerability blocks release unless a separately approved, expiring exception documents non-exploitability and compensation.

## Residual risks

- Same-user filesystem races cannot be fully eliminated portably in Node.js; scope is reduced by canonicalization, opened-handle validation, bounded read, and a local single-user deployment assumption.
- Native image decoding remains a memory-safety and availability risk despite format and resource limits.
- Provider output can still influence a host model. Sight MCP can label and structure data but cannot enforce the host's later behavior.
- Remote provider policy and retention cannot be technically enforced by Sight MCP.
- An operator can intentionally configure an overly broad allowed root or a remote provider; warnings cannot replace operator judgment.

## Review triggers

Update or supersede this threat model before adding URL/base64 input, archives, SVG/PDF/video, server-side capture, caches, persistent logs, telemetry, multiple providers, automatic failover, Streamable HTTP, authentication, remote hosting, or multi-tenant use.
