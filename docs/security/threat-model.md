...
- 修订：2026-09-17，依据 Issue #63（移除内置 profiles，通用 Provider 配置）
- 运行时风险：高
- 范围：本地 stdio 服务、本地文件与 macOS 剪切板图像输入、`sharp`
  预处理、一个由环境变量配置的 OpenAI 兼容视觉 Provider，以及可选的 macOS Keychain 凭据
...
- Provider 端点/模型配置与 macOS Keychain 条目元数据。
...
- macOS Keychain 与 `securityd`：由 `SIGHT_PROVIDER_KEYCHAIN_ACCOUNT`
  显式启用时使用的操作系统凭据边界。
...
| FS-05   | 恶意或配置错误的客户端上报过宽的工作区根目录       | 允许根边界被整体绕过          | 拒绝文件系统根与整个主目录并告警；对每个根执行 realpath 与目录校验；仅接受 `file://` scheme；把嵌套根折叠为最小集合       | 针对过宽根、非 file scheme、不存在目录与嵌套折叠的单元测试     |
| FS-06   | 会话授权缓存把一次批准放大成过宽的持续授权         | 未预期目录的无关文件泄露      | 只缓存用户实际批准路径的父目录，从不向上提升；不缓存文件系统根与主目录；仅存于进程内存不落盘；取消与拒绝都不写入缓存      | 针对目录粒度、拒绝不缓存、中止不残留与重启后失效的单元测试     |
...
| CRED-02 | 密钥与错误的 Provider 端点或模型配对               | 认证失败或非预期泄露          | 端点与模型仅来自校验过的环境配置；Keychain 仅在显式命名账户时读取；无自动回退                                             | 优先级与命名账户解析测试                                       |
...
| CRED-04 | 过宽或非预期的删除移除了其它凭据                   | 凭据丢失                      | 精确的 service 与校验过的账户名格式；每条命令只针对一个账户；除非 `--yes` 否则交互确认                                    | CLI 解析器与删除确认测试                                       |
| CRED-05 | Keychain 缺失、锁定或不受支持导致不安全回退        | 使用了错误的凭据/Provider     | Keychain 仅在显式配置 `SIGHT_PROVIDER_KEYCHAIN_ACCOUNT` 时使用；失败即中止启动；环境变量优先级固定；不尝试第二个 Provider | 缺失/不可用/命令失败的启动测试                                 |
...
8. 配置了 `SIGHT_PROVIDER_KEYCHAIN_ACCOUNT` 时，API 密钥从精确的 Keychain
   service 与该账户请求，保存在有界的进程内存中，由同一套脱敏密钥类型包装，且仅用于该 Provider 请求。Sight
   MCP 不会把它缓存到磁盘。
...
- 已存储的密钥在启动期间会短暂存在于托管的 Node.js 内存中，无法保证被立即清零。
...
HTTP、认证、远程托管、多租户使用、另一种操作系统凭据后端、非 macOS 剪切板后端、系统原生截屏或服务端捕获、持久化剪切板缓存、静默或缓存同意的剪切板读取，以及对凭据优先级的更改。
