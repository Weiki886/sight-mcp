...
**语言 / Language：** [中文](threat-model.md) · English
...
| FS-05   | Malicious or misconfigured client reports an over-broad workspace root  | Allowed-root boundary bypassed wholesale                | Refuse the filesystem root and the whole home directory with a warning; realpath and directory-check every root; accept only the `file://` scheme; collapse nested roots                                                      | Unit tests for broad roots, non-file schemes, missing directories, and nested collapsing                     |
| FS-06   | Session authorization cache widens one approval into a broad grant      | Disclosure of unrelated files in unexpected directories | Cache only the parent directory of the path the user actually approved and never walk upward; never cache the filesystem root or home directory; keep grants in process memory only; record nothing on denial or cancellation | Unit tests for directory scope, denial not cached, abort leaving no grant, and no persistence across restart |
