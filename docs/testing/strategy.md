...
- 各日志级别与错误路径下的密钥脱敏。
- 凭据优先级：`SIGHT_PROVIDER_API_KEY` 环境变量优先，其次是 `SIGHT_PROVIDER_KEYCHAIN_ACCOUNT`
  指定的 Keychain 账户；
- 设置了环境密钥时绝不读取 Keychain，且 Keychain 查询缺失/失败时失败即关闭；
- 未配置任何密钥时按无鉴权端点处理。

### 凭据 CLI 与 macOS Keychain

- CLI 解析器只接受有文档记录的凭据命令语法与合法的账户名，非法输入返回用法状态码 `2`；
- 已移除的 `--provider` 参数以状态码 `2` 退出并输出迁移指引，且不回显输入值；
...
- 环境变量配置（含环境密钥）能够初始化并列出工具，而已移除的 `--provider`
  参数与未知参数在输出协议 stdout 之前即以状态码 `2` 退出；
...
至少一次 release-candidate 冒烟运行使用本地 OpenAI 兼容端点。发布前还需要一次经用户授权的远程冒烟测试（`--live`
模式，使用非敏感的合成图表/OCR 夹具，端点/模型/密钥全部由运行器环境变量注入），并且至少要有一条 Claude
Code 记录和一条 Codex 记录以该模式启动打包制品。自然语言答案按能力进行人工审阅，而不做逐字节断言。

在 macOS 上，需单独验证真实的 Keychain 边界：在一个隔离的测试 service 名下使用生成的 canary，交互式写入、在不泄露的前提下检查存在性、在内存中读取并比对、删除该精确条目，并确认其已不存在。清理结果是脱敏测试记录的一部分。绝不为此项检查覆盖或读取用户生产环境的生产账户条目。
